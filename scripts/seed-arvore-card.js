/**
 * Cria (uma única vez) o card "Registrar Árvore" na coleção /cards, usado pelo
 * piloto do módulo Arvoredo do Bairro. Idempotente: não duplica se o card já
...
const ROTA_CARD = '/arvores/nova';

const CARD_ARVORE = {
  icon: 'park',
  title: 'Registrar Árvore',
  info: 'Fotografe e cadastre uma árvore do bairro',
...
async function main() {
  ...
  if (!existente.empty) {
    console.log(`Card "Registrar Árvore" já existe (id: ${existente.docs[0].id}). Nada a fazer.`);
    return;
  }
  ...
  const docRef = await cardsRef.add({ ...CARD_ARVORE, ordem: proximaOrdem });
  console.log(`Card "Registrar Árvore" criado com sucesso (id: ${docRef.id}, ordem: ${proximaOrdem}).`);
}
